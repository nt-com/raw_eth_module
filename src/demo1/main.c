 ...
 * Description: Example Usage of RAW_ETH
 * nmt @ NT-COM
 ***************************************************/
...
int main(int argc, char **argv) {
	...
	/* assemble a short ethernet frame */
	frame_size = raw_eth_assemble(&dmac[0], &send_payload[0], RAW_ETH_MIN_PAYLOAD_SIZE);	
	if(frame_size > 1) {
		/* send the frame */
		if(raw_eth_send(&sockfd, frame_size)) {
			printf("error sending\n");
